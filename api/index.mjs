...
function rateLimiter(config) {
  ...
  return async (c, next) => {
    ...
    if (record.count > maxRequests) {
      c.header("Retry-After", String(Math.ceil((record.resetAt - now) / 1e3)));
...
var strictRateLimit = rateLimiter({
  windowMs: 60 * 60 * 1e3,
  // 1 hora
  maxRequests: 3
  // 3 intentos (password reset, etc.)
});
...
var AccountLockout = class {
  ...
  isLocked(lockedUntil) {
    ...
    return /* @__PURE__ */ new Date() < lockedUntil;
  }
};
...
// src/domain/entities/Architecture.ts
var Architecture = class _Architecture {
  constructor(props) {
    this.props = props;
  }
  props;
  ...
  static fromPersistence(props) {
    return new _Architecture(props);
  }
  get id() {
...
// src/application/use-cases/architecture/CreateArchitecture.ts
var CreateArchitecture = class {
...
// src/application/use-cases/architecture/UpdateArchitecture.ts
var UpdateArchitecture = class {
  constructor(architectureRepository) {
    this.architectureRepository = architectureRepository;
  }
  architectureRepository;
  async execute(userId, architectureId, input) {
...
// src/application/use-cases/architecture/DeleteArchitecture.ts
var DeleteArchitecture = class {
  constructor(architectureRepository) {
    this.architectureRepository = architectureRepository;
  }
  architectureRepository;
  async execute(userId, architectureId) {
    const architecture = await this.architectureRepository.findById(architectureId);
    if (!architecture) {
      return err("Arquitectura no encontrada");
    }
...
// src/application/use-cases/architecture/GetArchitecture.ts
var GetArchitecture = class {
  constructor(architectureRepository) {
    this.architectureRepository = architectureRepository;
  }
  architectureRepository;
...
// src/application/use-cases/architecture/ListUserArchitectures.ts
var ListUserArchitectures = class {
  constructor(architectureRepository) {
    this.architectureRepository = architectureRepository;
  }
  architectureRepository;
...
function getGlobalConfig(config2) {
  return {
    lang: config2?.lang ?? store2?.lang,
    message: config2?.message,
    abortEarly: config2?.abortEarly ?? store2?.abortEarly,
    abortPipeEarly: config2?.abortPipeEarly ?? store2?.abortPipeEarly
  };
}
var store22;
function getGlobalMessage(lang) {
  return store22?.get(lang);
}
var store3;
...
function validate(schema, target = "json") {
  return async (c, next) => {
    let data;
    try {
      switch (target) {
...
var ArchitectureTypeSchema = picklist([
  "monolithic",
  "modular",
  "feature-based",
  "atomic-design",
...
var NodePropSchema = object({
  name: pipe(string(), maxLength(100)),
  type: pipe(string(), maxLength(100)),
  required: boolean(),
  defaultValue: optional(pipe(string(), maxLength(500)))
});
var NodeStateSchema = object({
  name: pipe(string(), maxLength(100)),
  type: pipe(string(), maxLength(100)),
  initialValue: optional(pipe(string(), maxLength(500)))
});
var NodeEffectSchema = object({
  name: pipe(string(), maxLength(100)),
  dependencies: pipe(
    array(pipe(string(), maxLength(100))),
    maxLength(50)
  ),
  cleanup: boolean()
});
var NodeDataSchema = object({
  label: pipe(string(), minLength(1), maxLength(100)),
  type: NodeTypeSchema,
  description: optional(pipe(string(), maxLength(500))),
  props: optional(pipe(array(NodePropSchema), maxLength(50))),
  state: optional(pipe(array(NodeStateSchema), maxLength(50))),
  effects: optional(pipe(array(NodeEffectSchema), maxLength(20)))
});
var ArchitectureNodeSchema = object({
  id: pipe(string(), maxLength(50)),
  type: NodeTypeSchema,
  position: PositionSchema,
  data: NodeDataSchema,
  parentId: optional(pipe(string(), maxLength(50)))
});
var EdgeDataSchema = object({
  type: EdgeTypeSchema,
  label: optional(pipe(string(), maxLength(100))),
  animated: optional(boolean())
});
var ArchitectureEdgeSchema = object({
  id: pipe(string(), maxLength(50)),
  source: pipe(string(), maxLength(50)),
  target: pipe(string(), maxLength(50)),
  sourceHandle: optional(string()),
  targetHandle: optional(string()),
  data: EdgeDataSchema,
  animated: optional(boolean())
});
var CreateArchitectureSchema = object({
  name: pipe(
    string(),
    trim(),
    minLength(1, "Nombre requerido"),
  ...
  description: pipe(
    string(),
    trim(),
    maxLength(500, "Descripcion muy larga")
  ),
  nodes: pipe(
    array(ArchitectureNodeSchema),
    maxLength(500, "Maximo 500 nodos")
  ),
  edges: pipe(
    array(ArchitectureEdgeSchema),
    maxLength(1e3, "Maximo 1000 conexiones")
  )
});
var UpdateArchitectureSchema = partial(CreateArchitectureSchema);
var PaginationSchema = object({
  page: optional(
    pipe(unknown(), transform(Number), number(), integer(), minValue(1)),
    1
  ),
  limit: optional(
    pipe(
...
var ObjectIdSchema = pipe(
  string(),
  regex(/^[a-f\d]{24}$/i, "ID invalido")
);

// src/infrastructure/http/controllers/ArchitectureController.ts
var ArchitectureController = class {
  createArchitecture;
  updateArchitecture;
  deleteArchitecture;
  getArchitecture;
  ...
  async create(c) {
    ...
    const input = getValidatedData(c);
  ...
  async list(c) {
    ...
    const paginationResult = safeParse(PaginationSchema, query);
    const pagination = paginationResult.success ? paginationResult.output : { page: 1, limit: 20 };
...
// src/infrastructure/http/routes/architecture.routes.ts
function createArchitectureRoutes(architectureRepository, jwtService) {
...
var User = class _User {
  ...
  isLocked() {
    ...
    return /* @__PURE__ */ new Date() < this.props.lockedUntil;
  }
  resetLoginAttempts() {
...
var PasswordSchema = pipe(
  string(),
  minLength(12, "Minimo 12 caracteres"),
...
var NameSchema = pipe(
  ...
  regex(/^[\p{L}\p{M}\s'-]+$/u, "Caracteres no permitidos")
);
...
var LoginSchema = object({
  ...
  password: pipe(
    string(),
    minLength(1, "Password requerido"),
    maxLength(128)
  )
});
var RefreshTokenSchema = object({
  refreshToken: pipe(string(), minLength(1, "Refresh token requerido"))
});

// src/infrastructure/http/routes/auth.routes.ts
function createAuthRoutes(userRepository, passwordService, jwtService) {
  const router = new Hono2();
...
// src/application/use-cases/user/DeleteUser.ts
var DeleteUser = class {
  constructor(userRepository, architectureRepository) {
    this.userRepository = userRepository;
    this.architectureRepository = architectureRepository;
  }
  userRepository;
...
var UserController = class {
  ...
  updateUser;
  deleteUser;
  constructor(userRepository, architectureRepository, passwordService) {
...
var UpdateUserSchema = pipe(
  object({
    name: optional(NameSchema),
    avatar: optional(
      pipe(string(), url("URL invalida"), maxLength(500))
    ),
    currentPassword: optional(string()),
    newPassword: optional(PasswordSchema)
  }),
  check((input) => {
    if (input.newPassword && !input.currentPassword) {
      return false;
...
function createUserRoutes(userRepository, architectureRepository, passwordService, jwtService) {
  const router = new Hono2();
...
// src/infrastructure/persistence/mongodb/models/ArchitectureModel.ts
import mongoose, { Schema } from "mongoose";
var PositionSchema2 = new Schema(
  {
...
var NodePropSchema2 = new Schema(
  {
...
var NodeStateSchema2 = new Schema(
  {
    name: { type: String, required: true },
    type: { type: String, required: true },
...
var NodeEffectSchema2 = new Schema(
  {
    name: { type: String, required: true },
...
var NodeDataSchema2 = new Schema(
  {
    ...
    props: [NodePropSchema2],
    state: [NodeStateSchema2],
    effects: [NodeEffectSchema2]
  },
  { _id: false }
);
var ArchitectureNodeSchema2 = new Schema(
  {
    ...
    position: { type: PositionSchema2, required: true },
    data: { type: NodeDataSchema2, required: true },
    parentId: { type: String }
  },
  { _id: false }
);
var EdgeDataSchema2 = new Schema(
  {
    type: {
      type: String,
      required: true,
...
var ArchitectureEdgeSchema2 = new Schema(
  {
    id: { type: String, required: true },
    ...
    data: { type: EdgeDataSchema2, required: true },
    animated: { type: Boolean }
  },
  { _id: false }
);
var ArchitectureSchema = new Schema(
  {
    ...
    nodes: [ArchitectureNodeSchema2],
    edges: [ArchitectureEdgeSchema2],
    isPublic: {
      type: Boolean,
      default: false
    }
...
var ArchitectureModel = mongoose.model(
  "Architecture",
  ArchitectureSchema
);

// src/infrastructure/persistence/mongodb/repositories/MongoArchitectureRepository.ts
var MongoArchitectureRepository = class {
  async findById(id) {
  ...
  async findByUserId(userId, options) {
    ...
    const [docs, total] = await Promise.all([
      ArchitectureModel.find({ userId }).sort({ createdAt: -1 }).skip(skip).limit(limit).lean().exec(),
...
// src/infrastructure/persistence/mongodb/models/UserModel.ts
...
// src/infrastructure/persistence/mongodb/repositories/MongoUserRepository.ts
var MongoUserRepository = class {
  async findById(id) {
  ...
  async findByEmail(email2) {
    const doc = await UserModel.findOne({ email: email2.toLowerCase() }).lean().exec();
    if (!doc) return null;
    return this.toDomain(doc);
  }
  async save(user) {
...
var JwtService = class {
  ...
  async generateAccessToken(userId) {
    return new SignJWT({ sub: userId, type: "access" }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("15m").sign(this.accessSecret);
  }
  async generateRefreshToken(userId) {
    return new SignJWT({ sub: userId, type: "refresh" }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("7d").sign(this.refreshSecret);
  }
...
function createApp(env) {
  ...
  app2.get("/api/health", (c) => c.json({ status: "ok" }));
  ...
  app2.route("/api/users", userRoutes);
  app2.route("/api/architectures", architectureRoutes);
  app2.onError((err2, c) => {
    console.error(err2);
  ...
  app2.notFound((c) => c.json({ error: "Endpoint not found" }, 404));
  return app2;
}
...
var EnvSchema = object({
  ...
  // Database
  MONGODB_URI: pipe(string(), url()),
  // JWT Secrets (minimo 32 caracteres)
  JWT_ACCESS_SECRET: pipe(string(), minLength(32)),
  JWT_REFRESH_SECRET: pipe(string(), minLength(32)),
  // CORS
  CORS_ORIGIN: string()
});
function loadEnv() {
  const result = safeParse(EnvSchema, {
    ...
    CORS_ORIGIN: process.env.CORS_ORIGIN ?? "https://fsa-eosin.vercel.app,http://localhost:3000"
  });
  if (!result.success) {
    const messages = result.issues.map(
      (issue) => `${issue.path?.map((p) => p.key).join(".") ?? "unknown"}: ${issue.message}`
    );
...
async function connectDatabase(uri) {
  ...
  try {
    ...
    mongoose3.connection.on("error", (err2) => {
      console.error("MongoDB connection error:", err2);
    });
